...
//        }
    ...
    {
        // Retrieve the user's profile
        $profile = Profile::where('applicant_id', $userId)->with('qualification','experience', 'post')->first();
...
//                    dd( $guesser->guess( storage_path('app/public/' . $profile->resume_path)));
...
//        return response()->download($pdfFilePath)->deleteFileAfterSend(true);
    ...
    protected function convertImageToPdf($imagePath, $outputPath)
    {
        ...
        $html = '<html><body><img src="' . $imagePath . '" style="width:100%; height:auto;"/></body></html>';
    ...
    public function downloadUserDocumentst($userId)
    {
        $profile = Profile::where('applicant_id', $userId)->with('qualification', 'experience', 'post')->first();
...
// Step 1: Create a new Merger instance
        $merger = new Merger();

// Step 2: Render the Blade view to HTML and create a PDF for the profile
        $viewContent = \Illuminate\Support\Facades\View::make('candidate_profile_print_new', compact('profile'))->render();
        $mpdf = new Mpdf();
        $mpdf->WriteHTML($viewContent);
        $mainPdfPath = storage_path('app/public/temp_main.pdf');
        $mpdf->Output($mainPdfPath, \Mpdf\Output\Destination::FILE);

// Add the main profile PDF to the merger
        $merger->addFile($mainPdfPath);

// Step 3: Check and add the resume PDF if it exists
        $resumePath = storage_path('app/public/' . $profile->resume_path);
        if (file_exists($resumePath) && pathinfo($resumePath, PATHINFO_EXTENSION) == 'pdf') {
            $merger->addFile($resumePath);
        }

// Step 4: Merge the PDFs and save the result
        $combinedPdfPath = storage_path('app/public/uploads/pdf/user_documents_' . $profile->first_name . '.pdf');
        file_put_contents($combinedPdfPath, $merger->merge());

// Return the combined PDF to the browser or any further action you need
        return response()->file($combinedPdfPath, [
            'Content-Type' => 'application/pdf',
            'Content-Disposition' => 'inline; filename="user_documents_' . $profile->first_name . '.pdf"',
        ]);
    }
    ...
    public function downloadUserDocumentsm($userId)
    {
        // Retrieve the user's profile
        $profile = Profile::where('applicant_id', $userId)->with('qualification','experience')->first();
